...
def get_generated_graph(api_key, df, prompt):
    ...
    padded_prompt = f"""We will use your output directly to put into a dash plotly app, so only return code. 
        ...
        only extract that component. The graph should be based on: {prompt}, use plotly express, px, to create 
        the figure and give it template='plotly_dark', also give the dcc.Graph component:
